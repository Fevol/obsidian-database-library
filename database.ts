...
export class Database<T> extends EventComponent {
			...
			await this.persist.ready(async () => {
				...
				this.registerEvent(this.plugin.app.vault.on("modify", async (file) => {
					if (file instanceof TFile && file.extension === "md") {
						...
						const state =
							(current_editor && current_editor.file?.path === file.path && current_editor.editor) ?
	...
	async persistMemory() {
		...
		await Promise.all(
			Array.from(this.deleted_keys.values()).map(async (key) => await this.persist.removeItem(key)),
		);
